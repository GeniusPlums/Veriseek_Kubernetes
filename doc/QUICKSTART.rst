...
.. code-block:: shell
  ...
  kubectl get secret bastion-ssh-key -n VeriseekEducation-global-prod  -o json | jq  '.data | map_values(@base64d)' |   jq -r 'keys[] as $k | "export \($k|ascii_upcase)=\(.[$k])"'

  # 2.) save the private key to a file on your local dev machine and set permissions as required by AWS
  vim ~/.ssh/bastion.service.veriseekeducation.com.pem
  sudo chmod 400 ~/.ssh/bastion.service.veriseekeducation.com.pem

  # 3.) connect to the bastion server via ssh
  ssh bastion.service.veriseekeducation.com -i ~/.ssh/bastion.service.veriseekeducation.com.pem
...
.. code-block:: shell
  ...
  mysql -h mysql.service.veriseekeducation.com -u root -p

  mongo --port 27017 --host mongo.service.veriseekeducation.com

  redis-cli -h redis.service.veriseekeducation.com -p 6379
